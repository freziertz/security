# SURA YA KWANZA

Hiki kitabu kinakufundisha kuhusu usalama wako wakati unatumia mtandao kama vile kuperuzi tovuti mbalimbali, kufanya manunuzi mitandaoni, kutumia simu yako, kutumia application mbalimbali za simu kutumia email, kutumia mitandao ya kijamii, kuweka data zako kwenye mawinguni na matumizi ya vifaa vvya nyumbani vinavyotumia.

Kitabu hiki kimejikita kwenye mamba ambayo unatakiwa ufahamu kuhusu hatari za usalama na usiri wa taarifa zako mitandaoni kama vile online scams, typosquatting, spoofed website ni vingine vingi ili uweze kujikinga wewe binafsi na kuyaweka salama maeneo yako unayofanyia kazi.

**Kitabu Hiki ni Kwa Ajili ya Nani**

- kitabu hiki ni kwa ajili ya mtu yeyote ambaye anahitaji kujua zaidi kuhusu usama wake mitandaoni, mbali na kujua anatakiwawa afanye nini pia  kuelewa kwa nini anatakiwa afanye hivyo
- mtu yeyote anayetaka kuongeza uelewa wake juu ya vitu anavyo vifanya kila siku kama kufanya manunuzi mitandaoni, kutumia email, kuperuzi mitandaoni na kutumia mitandao ya kijamii.

Kumbuka kwamba kila kitu kilichojadiliwa kwenye kitabu hiki unaweza kukitumia kwenye maisha yako binafsi au kwenye maeneo ya kazi.

**Madhumini ya kitabu hiki ni kukuelekeza**

- Hatua ambazo unapaswa kuchukua ili kulinda kifaa chako na taarifa zako wakati unatamia mitandao
- Kutambu online scams, typosqutting, na spoonfed websites
- Kuelewa usalama wa vifaa unavyotembea navyo kama simu, tablet na laptop
- Kwwa nini unatakiwa kutumia VPN
- Jinsi gani ya kutumia mitandao ya kijamii na cloud storage kwa usama
- IoT ni nini na jinsi gani ya kuiweka salama



usalama ukiwa unaperuzi mtandaoni

usalama kwenye email

Usalama wa application mbali mbali

usalama kwenye network yako

usalama  wa mazingira yako

usalama wa 

# SURA YA PILI

# Usalama Wakati wa Kuperuzi Tovuti

Sehemu hii nitazungumzia zaidi usalama wa kuperuzi mitandaoni. Tuanze kwa kujiuliza kwanza mtandao salama ni upi? kwanini natakiwa nitumie mitandao salama hasa kwenye mambo kama ya manunuzi.

Lengo la kipnde hiki ni kujifunza

Tovuti salama ni ipi na jinsi gani ya kuitambua



Tovuti salama ni ile ambayo anuani (URL) yake inaanza na https na pia utaona alama ya kufuli upande wa kushoto wa anuani. s kwenye https inasimama kuwakilisha neno secure.

![File:HTTPS and padlock in website address bar.jpg - Wikimedia Commons](data:image/jpeg;base64,)

*Secure* maana yake nini?



HTTPS ni nini na jinsi gani  inalinda taarifa (data) zako 

Kabla ya kufanya manunuzi kwenye mtandao untatikwa uhakikishe kwamba HTTPS na kufuli la kijani liko kwenye anuani ya tovuti

Https maana yake nini na nini umuhimu wa hicho kikufuli kidogo? kabla ya kujibu swali hili, kwanza tunahitaji kujua yafuatayo; SSL Cerificate  ni nini na inafanyaje kazi

HTTPS ni toleo lililo salama la HTTP, hii ni protocol ambayo inatumika kwa ajili ya mawasilino kati ya browser (client) na server. hiyo "S" iliyopo mwisho wa HTTPS inawakilisha neno *secure* maana yaka salama.

Kitaalamu inamaanisha HTTP ambayo imejengwa na Secure Socket Layer (SSL)

HTTPS maana yake mawasiliano yote kati yako yaani browser na web server yanakuwa encrypted. 

Nyuma ya pazia la HTTPS, SSL Certificate inafanya kazi muhimu ya kujenga uaminifu kati ya browser na web server. Kwa taarifa yako SSL certificate maana yake saini ya kidigitali ya web server ambayo inatolewa na mamlaka zilizopewa idhini ya kuzitoa ambayo inathibitisha utambulisho wa sebserver

TLS handshake ni nini?

![HTTPS](https://www.seobility.net/en/wiki/images/thumb/f/fa/HTTPs.png/450px-HTTPs.png)





HTTP

HTTPS

HTTPS ni kifupi cha Hypetext Transfer Protocol Secure ni protocol ya mawasiliano kwa ajili ya tovuti ambapo encryption protocol Secure Sockets Layer (SSL) au Transport Layer Security (TLS) encrypt taarifa (data)

Hypertext

Protocol



[Hypertext](https://www.seobility.net/en/wiki/Hypertext) Transfer Protocol Secure (HTTPS) is a communication protocol for websites in which the encryption protocol Secure Sockets Layer (SSL) or the Transport Layer Security (TLS) encrypts the data

